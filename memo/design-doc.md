...
- デフォルトのリンクテキスト:
  ...
  - 注釈 → `注釈 n`
- 同じグループ内の別ドキュメントを参照する場合は `@<ファイル名>/<ラベル>` を使用する。例: `@semantic-func/lem-substitution`。参照先は `RefLink` により現在のページ内参照と同じフローティング／インラインプレビューで表示される。
...
- **Markdown 記法**:
  ...
  - 推論木（自然演繹など）は `:::prooftree` ディレクティブから Typst 文字列を生成し、内部に `rule(...)` の構造を書いて **内蔵の `rule` / `prooftree` マクロ（Typst prolog で定義）** で描画する。Typst Universe の `curryst` パッケージを使わずとも同じ構造で記述できる。
  - 配置ルール:
    - `:::annotation` と閉じ `:::` はそれぞれ単独行に配置する。
    - 直前の行に本文がある場合は同じ段落の末尾にマーカーを差し込み、空行なら新しい段落を起こす。
    - 閉じ `:::` の直後が本文行であればマーカー直後に文章を連結し、空行であれば後続を新しい段落として保持する。
  - 例:
    ```md
