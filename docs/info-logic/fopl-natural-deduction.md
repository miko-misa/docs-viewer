...
原子式を原子命題のように扱い、量化を無視すれば基本的には命題論理と自然演繹と同じである。ただし、量化子に関する特別な推論規則が必要になる。ここでは、命題論理に対する自然演繹法に対して異なる点を中心に説明する。
...
これは以下のように存在量かに関する規則として書くこともできる。
...
## 自然演繹法の完全性
一階述語論理に対する自然演繹法についても、命題論理の場合と同様に完全性（@duality-deductive/sec-soundness-completeness ）が成り立つ。証明は大変なのでここでは省略する。