...
## (sec-universe)= ユニバース
一階述語論理の **ユニバース(universe)** は、解釈において対象となる要素の集合であり、通常は非空集合として定義される。一階述語論理では個体変数が存在したが、この個体変数が何を指示しているのかはユニバースによって決定される。また、$forall x$という量化がどの集合全体での普遍性を表しているのかもユニバースによって決定される。
...
一階述語論理においても結合子の意味論は命題論理のものと同じなので同様の法則が成り立つ。加えて、一階述語論理特有の量化に関連する性質がいくつか存在する。
...
@thm-distribution-quantifiers-connectives では全称量化と連言、存在量化と選言の組み合わせではよい性質があった。 ここでは、全称量化と選言、存在量化と連言の組み合わせについて言えることがある。
...
## 冠頭標準形
命題論理にも標準形があったように、一階述語論理にも標準形が存在する。それが **冠頭標準形(prenex normal form)** である。冠頭標準形とは、すべての量化が式の先頭にまとまっている形のことである。たとえば、以下のような形である。
$$
forall x exists y forall z ( phi )
$$
ここで、$phi$は量化を含まない式である。下でその存在を示すが、証明そのものがその構成方法を与えている。

:::column-toc
(thm-prenex-normal-form)=
@title: 【定理】冠頭標準形の存在
**【主張】**
任意の一階述語論理式$phi in italic("Form")$に対して、冠頭標準形$psi in italic("Form")$が存在して、
$$
...
**【証明】**
帰納法で示す。まず、$phi$が量化を含まない場合、$phi$自体が冠頭標準形であるので、$psi = phi$とすればよい。
次に、$phi$が量化を含む場合を考える。仮定法の仮定から$sigma, tau in italic("Form")$が存在して、量化を含まないとする。
...
これが、スキーマではなく真に公理として認められるには @fopl-define/sec-abstract-fopl の @fopl-define/sec-higher-order で述べた通り「すべての述語$S$について」という量化が必要である。今回の形で言うなら「すべての$phi$について」を意味する量化になる。しかし、一階述語論理ではこのような高階の量化は扱えないため、スキーマとして公理を定義している。二階述語論理で可能になる。
