...
**束縛変数 (bound variable)** の概念は、論理式内の変数の出現に対してその量化の影響を表すものである。自由変数とは、式内で量化されていない変数のことであり、束縛変数とは式内で量化されている変数のことである。以下に例を示す。
...
### (sec-sentence)= 文・閉式
**文(sentence; closed formula; closed logical formula)** とは、自由変数を含まない、つまり$italic("FV")(phi) = emptyset$となる一階述語論理式である。つまり、文はすべての変数が量化されている式$phi$である。文全体の集合を$italic("Sent")$とする。
