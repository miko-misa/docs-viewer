...
$|{0,1}^NN| = |cal(C)|$を示す。ここで、$cal(C)$はカントル集合を表す。カントル集合とは、区間を 3 等分して中央の開区間を取り除く操作を無限に繰り返して得られる集合であり、初めは$C_0 = [0,1] = {x in RR | 0 <= x <= 1}$である。詳しくは以下のように定義される。
...
ここで、3 進法を考えると、$cal(C)$の元は 3 進法で表したときに 1 の数字を含まない数全体からなる集合である。これは補題にて後ほど示す。つまり、$cal(C)$の元$x$は
...
これは全単射であるから、$|QQ| = alef_0 = |NN|$が成り立つ。ここで、後に示す補題より、$|cal(P)(QQ)| = |cal(P)(NN)|$が成り立つ。
...
最後に、$|RR| <= |cal(P)(QQ)|$かつ$|cal(P)(NN)| <= |RR|$より、単射$f: RR -> cal(P)(NN)$と単射$g: cal(P)(NN) -> RR$が存在する。したがって、シュレーダー・ベルンシュタインの定理（補題として証明）より、全単射$h: RR -> cal(P)(NN)$が存在し、$|RR| = |cal(P)(NN)|$が成り立つ。$square.filled$

:::

:::column
@title: 【補題】カントル集合の 3 進展開の一意性

**【主張】**
カントル集合$cal(C)$の元$x$は 3 進展開
...
以上二つの命題より、カントル集合$cal(C)$の元$x$は 3 進展開
...
次にもし、ある$a_k$を選ぶことができるとき、$S$のなかで$a_k R a_(k+1)$が成り立つような元$a_(k+1) (!= a_(k))$を選ぶことができる。なぜなら、$a_k$は極小元ではないからである。これを繰り返すことで、無限降下列$(a_i)_(i in NN)$を構成することができる。:::annotation
...
この公理は、ZF 選択公理(ZFC)の公理系に含まれている。この公理は当たり前のことのように思える。この公理がなくても、$n$個の元$a_0, a_1, dots, a_(n-1)$を選ぶことはできる。しかし、これを繰り返して可算無限個の元を選ぶことは保証されない。現に、ZF の公理系のみでは、従属選択公理は証明できず可算無限個の元を選ぶことができない。
:::
...
累積帰納法(Strong induction)または完全帰納法(Complete induction)とは、数学的帰納法の一種であり、自然数全体の集合$NN$上で成り立つ命題$P(n)$がすべての自然数$n$に対して成り立つことを示すための方法である。累積帰納法は以下の 2 つのステップからなる。
...
次項で説明する構造的帰納法の説明のためにここで、**線形リスト(linear list)** について説明する。集合$A$上のリスト$"List"(A)$は以下のように再帰的に定義される。

1. $"nil": 1 -> "List"(A)$
   空リスト$[thin]$は線形リストである。
2. $"cons": A times "List"(A) -> "List"(A)$
   $a in A$と$l in "List"(A)$に対して、$"cons"(a, l)$は線形リストである。また、$a :: l$は$"cons"(a, l)$の別表記である。

たとえば、$A = NN$のとき、以下は$"List"(NN)$の元である。
...
構造的帰納法(Structural induction)とは、集合$A$上のリスト$L = "List"(A)$上で成り立つ命題$P(l)$がすべてのリスト$l in L$に対して成り立つことを示すための方法である。構造的帰納法は以下の 2 つのステップからなる。
...
また、$R$を$"List"(A)$上の関係として、以下のように定義すると、これは構造的帰納法となる。

$$
l_1 R l_2 <=> exists a in A, l in "List"(A) "s.t. " l_2 = a :: l "and " l_1 = l
$$
