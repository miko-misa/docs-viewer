...
命題論理の基本的な構成要素は **原始命題 (atomic proposition) ** である。原始命題は、真または偽のいずれかの値を持つ最小単位の命題であり、本書では $p_0, p_1, p_2, dots$ のように$p$の文字で表される。たとえば「今日は雨が降っている」や「2 は偶数である」などが原始命題の例である。また、$bot$(ボトム)は **偽 (false) ** を表す特別な原始命題であり、常に偽の値を持つ。
...
また、$top, bot$は **命題定数(propositional constants)** と呼ばれる。$bot$(ボトム)はすでに @sec-atom で説明したように偽を表す命題定数であり、$top$(トップ)は真を表す命題定数である。ただし、$top$は原始命題としては定義されていないが、$not bot$で表現できる。本書ではその略記として$top$を用いるときがある。また、$bot$も項をもたない論理結合子である。本書では単に結合子といった場合は$bot$は含まないものとする。ただし、議論として$bot$は含まれてないといけないので$bot$の議論は原始命題の方で行う。
:::annotation
特に、@set-relation/sec-inductionや@set-relation/sec-structural-inductionの帰納部において、$bot$を帰納部ではなく基底部で扱うことにしている。
:::
